...
The authoritative, prospectively frozen numerical protocol is
[`experiments/constrained_batch_shift/config.json`](experiments/constrained_batch_shift/config.json),
with a human-readable specification in the adjacent
[`README.md`](experiments/constrained_batch_shift/README.md). This plan records the
scientific role; the frozen experiment files control implementation details.
...
- the standard smooth spherical constraint with exact feasible fraction about 8.07%;
- eight predetermined late-stage BO states generated without shift-based selection.

## Non-Gaussian surrogate from the beginning

Use both a standard Gaussian control and a non-Gaussian main belief on the same
states and candidate batches.
...
Match the Gaussian control to the Student-t predictive mean and covariance so the
comparison isolates higher-order tails rather than location or scale changes. The
tail prior and resulting degrees of freedom are frozen and never tuned against the
diagnostic.
...
Generate exactly eight frozen late-stage states using the fixed seeds and procedure
...
4. fit both matched objective/constraint beliefs.
...
- at 512 standard QMC samples, this causes prospectively quantified acquisition-ranking, gradient, or optimizer-decision errors in at least four of eight states.

The exact material-error and cross-state prevalence thresholds are frozen in the
experiment contract and may not be changed after implementation or result inspection.
