...
T1--T4 are conditional on the reference law $P_{0,t,\theta}$ and therefore on
the hyperparameters used to define it.  If $\theta$ is updated, the relevant
reference covariance or precision and the concrete influence construction must
be recomputed.  The current theory does not bound errors arising from
approximate or factor-subsampled hyperparameter learning.
...
supplied below, and its locked prospective numerical pilot passed.  The
finite-sample end-to-end blocker is therefore closed for that exact
reflection-symmetry finite-grid instantiation; continuous-action optimization
...
and optimization error bounds are supplied.  The locked reflection-symmetry
finite-grid pilot now supplies one complete rigorous implementation; its scope
does not extend to continuous-action optimization or other inference backends.
