...
\[
    P_{0,t}(df) = p(df\mid \mathcal D_t)
\]
be a tractable reference law for the unknown objective $f$.  In the main paper
...
Assume the reference model admits a finite latent-Gaussian representation
\[
    \xi\sim\gamma_t=\mathcal N(m_t,K_t),
    \qquad
    f=\mathcal T_t\xi,
\]
so that $P_{0,t}=\mathcal T_{t\#}\gamma_t$.  Write each factor in latent
coordinates as
\[
    \varepsilon_j(\xi)=e_j(\mathcal T_t\xi).
\]
Partition $\xi$ into blocks $\xi=(\xi_1,\ldots,\xi_B)$.

For a weakly differentiable observable $g$, let
\[
    L(g)=(L_1(g),\ldots,L_B(g))^\top\in\mathbb R_+^B
\]
...
such that
\begin{equation}
    \left|\operatorname{Cov}_{\pi_{S,s}}(g,h)\right|
    \le L(g)^\top C_S L(h)
    \label{eq:screenable-covariance}
\end{equation}
for the required gap and factor observables, with the following computational
requirements:
...
gives
\[
    B_{\mathrm{struct}}(x,\widehat x)=L(F_{x,\widehat x})^\top w
\]
for all challengers.
...
\[
    b_j
    = L(e_j)^\top C_S L(F_{x_c,\widehat x}).
\]
